...
import {
  ...
  ChevronDown, ChevronUp, Flame, Calendar, ChefHat
} from "lucide-react";
...
function FaqItem({ q, a }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid #EFE7D8" }}>
      <button
      ...
      >
        <span style={{ fontSize: 16, fontWeight: 600, color: "#1C1410" }}>{q}</span>
        {open ? <ChevronUp size={18} color="#8A7B68" style={{ flexShrink: 0 }} /> : <ChevronDown size={18} color="#8A7B68" style={{ flexShrink: 0 }} />}
      </button>
      {open && <p style={{ fontSize: 15, color: "#5C5142", lineHeight: 1.65, paddingBottom: "1.1rem", margin: 0 }}>{a}</p>}
    </div>
  );
}
...
function FeatureCard({ icon, title, body }) {
  return (
    <div style={{ background: "#FFFFFF", border: "1px solid #EFE7D8", borderRadius: 18, padding: "1.5rem" }}>
      <div style={{ width: 42, height: 42, borderRadius: 12, background: "#FBF0E4", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14, color: "#D4742C" }}>
        {icon}
      </div>
      <p style={{ fontSize: 16, fontWeight: 700, color: "#1C1410", marginBottom: 6 }}>{title}</p>
      <p style={{ fontSize: 14, color: "#8A7B68", lineHeight: 1.6, margin: 0 }}>{body}</p>
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", gap: 18 }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
        <div style={{ width: 34, height: 34, borderRadius: "50%", background: "#1C1410", color: "#FFF8EF", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14 }}>
          {number}
        </div>
        {!isLast && <div style={{ width: 2, flex: 1, background: "#E5DDD0", marginTop: 6 }} />}
      </div>
      <div style={{ paddingBottom: isLast ? 0 : "2rem" }}>
        <p style={{ fontSize: 17, fontWeight: 700, color: "#1C1410", marginBottom: 4 }}>{title}</p>
        <p style={{ fontSize: 14.5, color: "#8A7B68", lineHeight: 1.6, margin: 0, maxWidth: 440 }}>{body}</p>
      </div>
    </div>
  );
}
...
export default function Landing({ onStart }) {
  return (
    <div style={{ fontFamily: "'Helvetica Neue', Arial, sans-serif", background: "#FAF6EE", color: "#1C1410", minHeight: "100vh" }}>

      {/* Nav */}
      <nav style={{ maxWidth: 1080, margin: "0 auto", padding: "1.5rem 1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 30, height: 30, borderRadius: 9, background: "#D4742C", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Flame size={17} color="#FFF8EF" />
          </div>
          <span style={{ fontSize: 17, fontWeight: 700 }}>BulkUp</span>
        </div>
        <button
          onClick={onStart}
          style={{ padding: "10px 18px", borderRadius: 12, border: "1px solid #1C1410", background: "transparent", color: "#1C1410", fontWeight: 600, fontSize: 14, cursor: "pointer" }}
        >
          Use our tracker
        </button>
      </nav>
      ...
      <header style={{ maxWidth: 720, margin: "0 auto", padding: "3rem 1.5rem 4rem", textAlign: "center" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#FBF0E4", color: "#A85420", padding: "6px 14px", borderRadius: 20, fontSize: 13, fontWeight: 600, marginBottom: "1.75rem" }}>
          <TrendingUp size={14} /> Built for people trying to gain weight, not lose it
        </div>
        <h1 style={{ fontSize: "clamp(34px, 6vw, 52px)", fontWeight: 700, lineHeight: 1.12, margin: "0 0 1.25rem", letterSpacing: "-0.01em" }}>
          Stop guessing.<br />Start gaining.
        </h1>
        <p style={{ fontSize: 18, color: "#5C5142", lineHeight: 1.6, maxWidth: 560, margin: "0 auto 2.25rem" }}>
        ...
        <button
          ...
          style={{
            ...
            border: "none", background: "#D4742C", color: "#FFF8EF", fontWeight: 700, fontSize: 16, cursor: "pointer",
          }}
        >
          Use our tracker <ArrowRight size={18} />
        </button>
        <p style={{ fontSize: 13, color: "#A89A86", marginTop: 12 }}>Free. No account needed. Works right in your browser.</p>
      </header>
      ...
      <section style={{ maxWidth: 640, margin: "0 auto", padding: "1rem 1.5rem 4rem" }}>
        <p style={{ fontSize: 13, fontWeight: 700, color: "#D4742C", letterSpacing: "0.05em", marginBottom: 8 }}>HOW IT WORKS</p>
        <h2 style={{ fontSize: 28, fontWeight: 700, marginBottom: "2.25rem" }}>Three steps to a plan built around you</h2>
        <StepRow
          number={1}
      ...
      <section style={{ maxWidth: 680, margin: "0 auto", padding: "1rem 1.5rem 4rem" }}>
        <p style={{ fontSize: 13, fontWeight: 700, color: "#D4742C", letterSpacing: "0.05em", marginBottom: 8 }}>QUESTIONS</p>
        <h2 style={{ fontSize: 28, fontWeight: 700, marginBottom: "1rem" }}>Frequently asked</h2>
        <div>
      ...
      <section style={{ maxWidth: 680, margin: "0 auto", padding: "1rem 1.5rem 5rem", textAlign: "center" }}>
        <div style={{ background: "#1C1410", borderRadius: 24, padding: "3rem 2rem" }}>
          <h2 style={{ fontSize: 26, fontWeight: 700, color: "#FFF8EF", marginBottom: 10 }}>Ready to stop guessing?</h2>
          <p style={{ fontSize: 15, color: "#C9BBA4", marginBottom: "1.75rem" }}>Set up your plan in under a minute.</p>
          <button
            onClick={onStart}
            style={{
      ...
      <footer style={{ textAlign: "center", padding: "2rem 1.5rem", color: "#A89A86", fontSize: 13 }}>
